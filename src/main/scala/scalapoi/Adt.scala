...
object Adt {
  ...
  def createDocument(name: String): PoiGrammer[HSSFWorkbook] =
    liftF[PoiAdtT, HSSFWorkbook](CreateDocument(name))

  def createSheet(name: String, wb: HSSFWorkbook): PoiGrammer[HSSFSheet] =
